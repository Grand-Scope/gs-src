...
const DEFAULT_OPTIONS: HyperspeedOptions = {
  ...
  colors: {
    ...
    leftCars: [0x00d2ff, 0x007bff, 0x3396ff],
    rightCars: [0x004a99, 0x1e3a8a, 0x0063cc],
    sticks: 0x00d2ff,
  }
};
...
export const Hyperspeed = forwardRef<HTMLDivElement, HyperspeedProps>(({ effectOptions, className }, ref) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const colorSet = i % 2 === 0 ? colors?.leftCars ?? DEFAULT_OPTIONS.colors!.leftCars : colors?.rightCars ?? DEFAULT_OPTIONS.colors!.rightCars;
